...
>
> 表述层包括视图层（js、jsp，css等）和控制层（servlet、handler）

### 3、SpringMVC的特点

- **Spring 家族原生产品**，与 IOC 容器等基础设施无缝对接
- **基于原生的Servlet**，通过功能强大的**前端控制器DispatcherServlet**，对请求和响应进行统一处理
- 对表述层各细分领域需要解决的问题进行了**全方位覆盖**，提供**全面解决方案**
- **代码清新简洁**，大幅度提升开发效率
...
> 只要cookie存在且存储的会话ID不变，那么session永远是同一个
...
**浏览器发出一次请求，由服务器的servlet处理，然后服务器进行重定向。所以转发也叫做服务器内转发**
...
**浏览器发出两次请求，第一次访问servlet，服务器自动向浏览器发送一个响应，然后浏览器自动发出第二次请求，访问重定向的路径。中间的操作对用户是透明的，这又叫做服务器外跳转**
...
1. 浏览器发出请求次数不同
2. 转发到的新请求是可以获取第一次的请求参数的，而重定向是由浏览器第二次发起的请求，无法获取第一次请求的参数
3. **转发时，浏览器地址栏显示的是第一次请求的地址；重定向是重新指向的那个地址**，所以为了解决刷新页面还是请求的转发前的地址，可以使用重定向跳转。
4. 转发不能跨域，只能跳转到当前应用的资源中；重定向可以跨域，不仅可以跳转到当前应用的其他资源，也可跳转到其他应用的资源中

### 5、视图控制器view-controller

当控制器方法中，仅仅用来实现页面跳转，即只需要设置视图名称，不需要进行一些逻辑处理时，可以将处理器方法使用view-controller标签进行表示。
...
">
    <context:component-scan base-package="com.xpf"></context:component-scan>
